...
void GameplayScene::LoadLevel(int index)
{
	...
	player = std::make_unique<PlayerCannon>(world, context, config.cannonX, config.cannonY);
...
void GameplayScene::HandleInput()
{
		...
		if (IsKeyPressed(KEY_ENTER)) {
			if (context.finishState == GameFinishState::Won) {
				if (levelIndex + 1 < LevelCatalog::Count()) {
					game.SwitchScene(std::make_unique<GameplayScene>(game, levelIndex + 1));
				}
				else {
				...
				game.SwitchScene(std::make_unique<GameplayScene>(game, levelIndex));
			}
		}
		return;
	}
	...
	else if (context.debugMode && IsKeyPressed(KEY_N) && levelIndex + 1 < LevelCatalog::Count()) {
...
void GameplayScene::SpawnProjectile()
{
	if (!player) return;
	...
	canSpawnProjectile = (distance > CHAIN_LINK_SPACING * METERS_PER_PIXEL);

	if (canSpawnProjectile) {
		if (projectiles.size() >= MAX_CHAIN_LINKS) {
			return;
		}
...
void GameplayScene::PullProjectiles(float deltaTime)
{
...
void GameplayScene::ConsumeLink()
{
	if (!player || !wreckingBall) return;
...
void GameplayScene::EnableChainCollisions()
{
	b2Filter filter;
	filter.groupIndex = 0;
...
void GameplayScene::CleanupProjectiles()
{
...
void GameplayScene::DrawDebugJoint(const b2DistanceJoint& joint)
{
